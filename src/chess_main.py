...
def load_images():
    ...
    for image in chessman_images:
        ...
        IMAGES[image] = pygame.transform.scale(pygame.image.load('../images/' + image + '.png'),
...
def draw_board(screen):
    ...
    for row in range(DIMENSION):
        for col in range(DIMENSION):
            ...
            # Draw a rectangle with the color to the screen(surface) with the size of the chess box from left to
...
def main():
    ...
    # Chess man valid moves
    valid_move = game_state.valid_move()
    # Flag if a move is made
    made_move = False
    # Load images
    load_images()
    ...
    while running:
        for p in pygame.event.get():
            if p.type == pygame.QUIT:
                ...
                if game_state.board[row][col] == '--' and len(player_clicks) == 0:
                    continue
                ...
                if len(player_clicks) == 2:
                    ...
                    if move in valid_move:
                        ...
                        player_clicks = [selected_chess_box]
            elif p.type == pygame.KEYDOWN:
                if p.key == pygame.K_z and pygame.key.get_mods() & pygame.KMOD_LCTRL:
